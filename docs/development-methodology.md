# Linfordroyan — Development Methodology

Adapted from [`development-methodology-template.md`](../development-methodology-template.md).

---

## Linfordroyan context

| Item | Decision |
|------|----------|
| **Phase** | Static site — frontend-only MVP |
| **Stack** | Angular SPA in `apps/client/` · PrimeNG · Tailwind CSS · static prerender for GitHub Pages |
| **Persistence** | Static content in repo (JSON/TS); Strapi CMS post-MVP |
| **Auth** | None — public portfolio |
| **Roles** | N/A |
| **Future stack** | Strapi headless CMS · blog & dynamic content |

### Delivery order

| Step | Deliverable | Status |
|------|-------------|--------|
| 0 | Project conventions + docs/ layout | Done |
| 1 | PRD (`docs/prd/`) | Done |
| 2 | Tech spec (`docs/spec/`) | Done |
| 3 | App scaffold via CLI (`ng new` in `apps/client/`) | Done |
| 4 | Shell: layout, theme, PrimeNG + Tailwind, GH Pages | Done |
| 5 | Hero · About · Contact · Resume download | Done (MVP skeleton) |
| 6 | Services · Skills | Done (MVP skeleton) |
| 7 | Experience timeline | Done (MVP skeleton) |
| 8 | Featured case studies (3 deep pages) | Done (MVP skeleton) |
| 9 | Projects grid · Certifications · Education | Done (MVP skeleton) |
| 10 | Testimonials placeholder · Google Analytics | Done (MVP skeleton) |
| 11+ | Strapi CMS integration · UI polish (`ui-ux-pro-max`) | **Next** |

---

## How to adopt in this project

1. PRD lives in `docs/prd/`.
2. Tech spec lives in `docs/spec/`.
...
| Item | Linfordroyan |
|------|--------------|
| Product name | **Linfordroyan** |
| PRD folder | `docs/prd/` |
| Tech spec folder | `docs/spec/` |
| API base path | N/A (static content for MVP) |
| Client app | Angular SPA in `apps/client/` |
| Server / ORM | none (static MVP); Strapi post-MVP |
| Unit test location | `*.spec.ts` beside source |
| E2E test location | `e2e/` |
| CI config | `.github/workflows/ci.yml` |
| ADR folder | `docs/adr/` |

### Site content map (from resume)

| Section | Source |
|---------|--------|
| Hero / About | CV summary — 7+ yrs, Angular/Vue/Node, architecture & SDD |
| Services | Angular specialist · Vue.js · Node.js/NestJS · technical leadership |
| Skills | Core competencies (grouped) |
| Experience | Codecraft, Ness, Getir, Simplilearn, Moonraft |
| Case studies | Financial MVP+SDD · Ness Angular 17 · Getir logistics |
| Certifications & Education | CV tail |
| Contact | linfordroyan05@gmail.com · +91 7795867758 · mailto CTA |
| Resume | `CV_LINFORDROYAN.pdf` download |

### Open decisions (resolve in PRD step 1)

| Topic | Default until decided |
|-------|------------------------|
| Case study selection & depth | 3 deep pages + project cards for remaining roles |
| PrimeNG + Tailwind integration | Document in `docs/spec/theming.md` |
| GitHub Pages prerender + routing | Document in `docs/spec/deployment.md` |
| Strapi content model | Post-MVP; blog + projects + testimonials first |
| GA cookie consent | Best-effort banner in MVP |
| Dark mode persistence | `localStorage` key `theme` |
| Custom domain | TBD post-launch |
...
4. **CLI for scaffolding** — use official CLIs (`ng new`, etc.); do not hand-roll boilerplate or manually bump versions.
...
| `.cursor/skills/create-project-methodology/SKILL.md` | Wizard to regenerate this doc |
| `.cursor/skills/ui-ux-pro-max/SKILL.md` | UI/UX design intelligence (styles, palettes, typography, UX guidelines) |

### UI/UX workflow

For UI design and implementation, invoke **`ui-ux-pro-max`** when building pages, choosing palettes, typography, or reviewing accessibility. Run design-system search via the skill's Python scripts before coding visual features.
...
**MVP note:** FDD slices are client-only (static content + UI). Spec content contracts so Strapi can replace static data later.
